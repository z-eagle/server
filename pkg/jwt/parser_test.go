	...
	},
	{
		...
		&StandardClaims{
			RegisteredClaims: goJWT.RegisteredClaims{
				ExpiresAt: goJWT.NewNumericDate(time.Now().Add(time.Second * 10)),
			},
		},
		true,
		0,
...
func mapClaimsToken(claims *MapClaims) string {
	...
	j := goJWT.NewWithClaims(goJWT.SigningMethodHS512, claims)
	...
	for _, data := range jwtTestData {
		...
		t.Run(data.name, func(t *testing.T) {
			...
			case *MapClaims:
				...
				_, err = ParseWithClaims(data.tokenString, &MapClaims{}, data.keyfunc)
			case *StandardClaims:
				if data.tokenString == "" {
			...
			if data.errors != 0 {
				...
				if !ok {
					t.Errorf("Expected *ValidationError, but got %#v instead", err)
				}
			}
		})
	}
